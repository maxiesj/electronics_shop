    ...
    if (!hash_equals($_SESSION['payroll_csrf'], (string)($_POST['csrf_token'] ?? ''))) {
        ...
        if ($action === 'save_salary') {
            ...
            if ($recordId <= 0 || !$isDate($paymentDate) || !in_array($method, $methods, true) || !preg_match('/^[A-Z0-9][A-Z0-9._\/-]{2,99}$/', $reference)) {
                ...
                $stmt = $conn->prepare("SELECT id FROM payroll_records WHERE reference_number=? AND id<>? LIMIT 1");
                $stmt->bind_param('si', $reference, $recordId);
                $stmt->execute();
                $referenceUsed = $stmt->get_result()->fetch_assoc();
                $stmt->close();
                if ($referenceUsed) {
