...
$trashCounts=['category'=>0,'brand'=>0];
if(trashRegistryAvailable($conn)){
    $trashCountResult=$conn->query("SELECT record_type,COUNT(*) total FROM trash_records WHERE status='trashed' AND record_type IN ('category','brand') GROUP BY record_type");
    while($trashCountResult&&$trashCountRow=$trashCountResult->fetch_assoc())$trashCounts[$trashCountRow['record_type']]=(int)$trashCountRow['total'];
}
?>
<style>
...
.taxonomy-panel__head{display:flex;justify-content:space-between;align-items:center;gap:14px}.taxonomy-panel__head>div{min-width:0}.taxonomy-trash-link{display:inline-flex;align-items:center;gap:6px;padding:8px 11px;border:1px solid #fecaca;border-radius:8px;background:#fff7f7;color:#b42318;text-decoration:none;font-size:12px;font-weight:850;white-space:nowrap}.taxonomy-trash-link:hover{background:#fee2e2}.taxonomy-trash-count{display:grid;min-width:20px;height:20px;padding:0 5px;place-items:center;border-radius:999px;background:#dc2626;color:#fff;font-size:10px}
@media(max-width:820px){.taxonomy-grid{grid-template-columns:1fr}.taxonomy-stats{grid-template-columns:repeat(2,1fr)}}@media(max-width:520px){.taxonomy-stats{grid-template-columns:1fr}.taxonomy-add{flex-direction:column}.taxonomy-row{grid-template-columns:1fr}.taxonomy-actions{justify-content:flex-end}.taxonomy-panel__head{align-items:flex-start}}
</style>
...
<section class="taxonomy-panel"><header class="taxonomy-panel__head"><div><h3><?=$title?></h3><p><?=count($items)?> registered · names must be unique</p></div><a class="taxonomy-trash-link ajax-link" data-target="trash.php?trash_type=<?=$kind?>" href="trash.php?trash_type=<?=$kind?>">Trash <span class="taxonomy-trash-count"><?=$trashCounts[$kind]?></span></a></header>
