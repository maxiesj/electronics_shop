...
if (empty($_SESSION['wallet_adjustment_csrf'])) $_SESSION['wallet_adjustment_csrf'] = bin2hex(random_bytes(32));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adjust_credit_action'])) {
    ...
    $amount = $amount_valid ? (float)$amount_raw : 0.0;
    if (!hash_equals($wallet_csrf, (string)($_POST['csrf_token'] ?? ''))) {
        ...
        try {
            $stmt = $conn->prepare('SELECT available_balance FROM customer_wallets WHERE id=? FOR UPDATE');
            ...
            if (!$stmt->execute()) throw new RuntimeException('Wallet update failed.');
            $stmt->close();
            $details = 'Wallet #' . $wallet_id . ': KES ' . number_format($before, 2) . ' to KES ' . number_format($after, 2);
...
$total_balance = 0.0;
$funded = 0;
$result = $conn->query('SELECT cw.id wallet_id,u.fullname,u.email,cw.available_balance,cw.updated_at FROM customer_wallets cw JOIN users u ON u.id=cw.user_id ORDER BY cw.available_balance DESC');
...
<td><form class='wallet-form' action='manage_wallets.php' method='post'><input type='hidden' name='adjust_credit_action' value='1'><input type='hidden' name='wallet_id' value='<?=(int)$row['wallet_id']?>'><input type='hidden' name='csrf_token' value='<?=htmlspecialchars($wallet_csrf)?>'><select name='adjustment_type'><option value='add'>Add credit</option><option value='deduct'>Deduct</option></select><input type='number' name='adjustment_amount' min='0.01' max='99999999.99' step='0.01' placeholder='KES 0.00' required><button type='submit'>Apply</button></form></td>
</tr>
...
(function(){
 ...
 document.querySelectorAll('.wallet-form').forEach(form=>form.addEventListener('submit',function(event){if(this.elements.adjustment_type.value==='deduct'&&!confirm('Confirm this wallet deduction?')){event.preventDefault();event.stopImmediatePropagation();return;}const button=this.querySelector('button');button.disabled=true;button.textContent='Applying...';}));
})();
</script>
